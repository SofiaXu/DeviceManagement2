  ...
  <div class="d-flex flex-column">
    ...
    <div class="row">
      <table class="table table-striped">
        <thead>
          <tr>            <th scope="col">用户类型ID</th>
            <th scope="col">用户类型名称</th>
<th scope="col">操作</th>
          </tr>
        </thead>
        <tbody>          <tr v-for="(model, index) in models" :key="index">
            <td>{{ model.userTypeId }}</td>
            ...
            <td>
              ...
              <button
                ...
                @click="deleteModel(model.userTypeId)"
              >
                🗑️
              </button>
            </td>
      ...
      >
        ...
        <template v-slot:default>          <div class="mb-3 row">
            <label class="col-sm-3 col-form-label">用户类型ID</label>
            <div class="col-sm-9">
              <input
                type="text"
                class="form-control"
                v-model="newModel.userTypeId"

              />
            </div>
          </div>          <div class="mb-3 row">
            <label class="col-sm-3 col-form-label">用户类型名称</label>
            <div class="col-sm-9">
              <input
                type="text"
                class="form-control"
                v-model="newModel.userTypeName"
        ...
        <template v-slot:default>          <div class="mb-3 row">
            ...
            <div class="col-sm-9">
              <input
                type="text"
                class="form-control"
                v-model="editableModel.userTypeId"
            ...
            <div class="col-sm-9">
              <input
                type="text"
                class="form-control"
                v-model="editableModel.userTypeName"
...
export default class UserTypeList extends Vue {
  private models: UserType[] = [];
  private searchText = "";
  private page: Pagination = new Pagination();
  private accessToken = store.state.userInfo.accessToken;
  private newModel: UserType = new UserType();
  private editableModel: UserType = new UserType();
  ...
  deleteModel(id: number) {
    ...
    fetch(`http://localhost:5000/api/userType/delete/${id}`, {
      method: "DELETE",
      headers: new Headers({
  ...
  getData() {
    fetch(
      `http://localhost:5000/api/userType?pageSize=${this.page.PageSize}&pageIndex=${this.page.CurrentPage}`,
      {
        method: "GET",
        headers: new Headers({
          Authorization: "Bearer " + this.accessToken,
      ...
      .then((responseJson) => {
        ...
        this.models = responseJson.content as UserType[];
      })
      .catch((error) => console.error(error));
  }
